...
def nove_povezave(pot, zemljevid):
    return set([x for x in pairwise(pot)]) - zemljevid.keys()


def obiskane_tocke(pot):
    return {x for x in pot}


def popravljena_pot(pot):
    return "".join(chain([x for x in pot[::2]], pot[-1]))


def povezave_z_vescino(pot, zemljevid, vescina):
    return [vmesna_pot for vmesna_pot in pairwise(pot) if vescina in zemljevid[vmesna_pot]]


def dolgocasna_pot(pot, zemljevid):
    return any(len(zemljevid[vmesna_pot]) == 0 for vmesna_pot in pairwise(pot))


def dobra_pot(pot, zemljevid):
    return all(len(zemljevid[vmesna_pot]) >= 2 for vmesna_pot in pairwise(pot))


def zahtevnost_poti(pot, zemljevid):
    return max(len(zemljevid[vmesna_pot]) for vmesna_pot in pairwise(pot))


def izvedljiva(pot, zemljevid, zivljenj):
    return zivljenj > len(set(pairwise(pot)) - set(zemljevid.keys()))


def enosmerne(zemljevid):
    return {vmesna_pot for vmesna_pot in zemljevid if (vmesna_pot[-1], vmesna_pot[0]) not in zemljevid}
